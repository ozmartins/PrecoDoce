...
def purchase_create(request):
    ...
    if not form.is_valid():
        ...
        return render(request, "purchase.html", context, status=400)
...
def purchase_update(request, pk: int):
    ...
    if not form.is_valid():
        if is_ajax(request):
            return JsonResponse({"ok": False, "errors": form.errors}, status=400)
        return render(request, "purchase/index.html", status=400)

    purchase = form.save()
    if is_ajax(request):
        return JsonResponse({"ok": True, "id": purchase.pk, "name": purchase.name})
    return redirect("purchases:purchase_recover")
