...
class RecipeCostSheetIngredient(models.Model):    
    ...
    total_cost = models.DecimalField(max_digits=10, decimal_places=2, validators=[MinValueValidator(Decimal('0.00'))])
    recipe_cost_sheet = models.ForeignKey(RecipeCostSheet, on_delete=models.CASCADE)
...
class RecipeCostSheetOtherCostsIngredient(models.Model):
    recipe_cost_sheet_other_costs = models.ForeignKey(RecipeCostSheetOtherCosts, on_delete=models.CASCADE)
    ingredient = models.ForeignKey(Ingredient, on_delete=models.RESTRICT)